...
class APIService {
    ...
    private init() {
        // Read base URL from Info.plist for environment configuration; fallback to local network address
        if let info = Bundle.main.infoDictionary, let configured = info["API_BASE_URL"] as? String, !configured.isEmpty {
            baseURL = configured
        } else {
            baseURL = "http://192.168.1.10:8000/api" // Local network API address with correct port
        }
    ...
    // MARK: - Generic Request Method
    func request<T: Codable>(endpoint: String, method: HTTPMethod, parameters: [String: Any]? = nil, completion: @escaping (Result<T, Error>) -> Void) {
        let url = "\(baseURL)\(endpoint)"
