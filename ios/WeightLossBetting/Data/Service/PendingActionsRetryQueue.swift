...
final class PendingActionsRetryQueue {
    ...
    init(repository: PendingActionsRepositoryProtocol = PendingActionsRepository()) {
        ...
        PendingActionsCoreStore.shared.migrateFromUserDefaults(oldQueueKey: "pending_network_actions_queue", oldSeenKey: "seen_pending_invitations")
    }
    ...
    private var queue: [PendingNetworkAction] {
        get { coreStore.fetchPendingNetworkActions() }
        set {
            // newValue should be written via coreStore methods individually; do nothing here
        }
    }

    func add(action: PendingNetworkAction) {
        coreStore.addPendingNetworkAction(action)
    }

    func remove(id: String, type: PendingActionType) {
        coreStore.removePendingNetworkAction(id: id, type: type)
    }

    func processQueue() async {
        var arr = coreStore.fetchPendingNetworkActions()
        guard arr.count > 0 else { return }

        for (index, var action) in arr.enumerated().reversed() {
            if action.attempt >= maxAttempts {
                // drop it
                coreStore.removePendingNetworkAction(id: action.id, type: action.type)
                continue
            }
            ...
            case .markSeen:
                ...
                case .failure(let error):
                    ...
                    action.attempt += 1
                    coreStore.updateAttempt(for: action.id, type: action.type, attempt: action.attempt)
                case .loading:
                    break
                }
