...
final class PendingActionsService {
    ...
    private let repository = PendingActionsRepository()
    ...
    private func handlePendingActions(_ response: PendingActionsResponse) async {
        // Handle invitations first
        if let invitations = response.invitations, invitations.count > 0 {
            for invite in invitations where (invite.isFirstTime ?? false) {
                // Check local seen set
                if hasLocallySeen(invitationId: invite.id) {
                    continue
                }
    ...
    private func presentInviteModal(_ invite: InvitationItem) async {
        await MainActor.run {
            ...
            vc.onDisplayed = { [weak self] in
                ...
                self?.markLocallySeen(invitationId: invite.id)
            }
            vc.onDismiss = { [weak self] success in
                ...
                if success {
                    ...
                    self.clearLocalSeen(invitationId: invite.id)
                }
            }
    ...
    private func presentDoubleCheckModal(_ dc: DoubleCheckItem) async {
        await MainActor.run {
            ...
            vc.onAction = { [weak self] action, comment in
                guard let self = self else { return }
                Task {
                    await self.postDoubleCheckWithRetry(planId: dc.planId, action: action, comment: comment)
                }
            }
    ...
    private func markInvitationSeenWithRetry(invitationId: String, attempt: Int = 1) async {
        ...
        case .success:
            ...
            // persist final seen
            persistFinalSeen(invitationId: invitationId)
        case .failure(let error):
            ...
            if attempt < maxRetryCount {
                ...
                clearLocalSeen(invitationId: invitationId)
    ...
    private func postDoubleCheckWithRetry(planId: String, action: String, comment: String?, attempt: Int = 1) async {
        ...
        let result = await repository.postDoubleCheck(planId: planId, parameters: parameters)
                // trigger background processing unless running in suppressed/test mode
                if !suppressUIModal {
                    Task {
                        await PendingActionsRetryQueue.shared.processQueue()
                    }
        case .failure(let error):
            ...
            if attempt < maxRetryCount {
                ...
                let netAction = PendingNetworkAction(id: planId,
                                                     ...
                                                     attempt: attempt,
                                                     createdAt: Date())
                PendingActionsRetryQueue.shared.add(action: netAction)
                Task {
                    await PendingActionsRetryQueue.shared.processQueue()
                }
            }
    ...
    private func hasLocallySeen(invitationId: String) -> Bool {
        return PendingActionsCoreStore.shared.hasSeenInvitation(id: invitationId)
    }

    private func markLocallySeen(invitationId: String) {
        PendingActionsCoreStore.shared.addSeenInvitation(id: invitationId)
    }

    private func clearLocalSeen(invitationId: String) {
        PendingActionsCoreStore.shared.removeSeenInvitation(id: invitationId)
    }

    private func persistFinalSeen(invitationId: String) {
        // For now same as local; could be extended to store in a different key
        markLocallySeen(invitationId: invitationId)
    }
