...
class CheckInViewController: UIViewController {
    ...
    private func loadActivePlan() {
        guard let userId = AuthRepository.shared.getCurrentUserId(), !userId.isEmpty else {
            return
        }
        
        viewModel.loadActivePlan(userId: userId) { [weak self] hasPlan in
            if !hasPlan {
                self?.showAlert(title: "提示", message: "暂无进行中的计划")
            }
        }
    }
    ...
    @objc private func submitTapped() {
        ...
        // 使用自动打卡，不需要传入 planId
        viewModel.createAutoCheckIn(weight: weight, note: note)
    }
