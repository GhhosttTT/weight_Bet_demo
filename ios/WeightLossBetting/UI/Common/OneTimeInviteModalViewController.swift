...
final class OneTimeInviteModalViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        messageLabel.text = invitation.message ?? "您已被邀请加入一个计划。"
    ...
    @objc private func viewTapped() {
        print("🔴 [InviteModal] 用户点击了查看计划按钮，planId=\(invitation.planId)")
        
        let planId = invitation.planId
        // 先发送通知，避免弹窗释放后无法发送
        print("📢 [InviteModal] 准备发送导航通知...")
        print("   - 通知名称：navigateToNotificationType")
        print("   - 类型：invite")
        print("   - planId: \(planId)")
        ...
        NotificationCenter.default.post(
            ...
            object: nil,
            userInfo: ["type": "invite", "relatedId": planId]
        )
        print("✅ [InviteModal] 通知已发送")
        
        // 再关闭弹窗
        dismiss(animated: true) { [weak self] in
            print("✅ [InviteModal] 弹窗已关闭")
            self?.onDismiss?(true)
        }
    }
